...
import {
  ...
  EmploymentType,
  CreateEmploymentTypeData,
} from '@/lib/api/org/employment-types';
...
export default function EmploymentTypesPage() {
  ...
  const [editingItem, setEditingItem] = useState<EmploymentType | null>(null);
  const [deletingItem, setDeletingItem] = useState<EmploymentType | null>(null);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');

  const [formData, setFormData] = useState<CreateEmploymentTypeData>({
    name: '',
    code: '',
    description: '',
    isActive: true,
  ...
  const loadData = async () => {
    try {
      setIsLoading(true);
      const data = await getAllEmploymentTypes(orgSlug);
      setEmploymentTypes(data);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Failed to load employment types');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  if (isLoading) {
    return (
        ...
        <PageLoader message="Loading employment types..." />
      
    );
  }

  const handleOpenDialog = (item?: EmploymentType) => {
    if (item) {
      setEditingItem(item);
      setFormData({
        name: item.name,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem) {
        await updateEmploymentType(orgSlug, editingItem.id, formData);
        toast.success('Employment type updated successfully');
      } else {
        await createEmploymentType(orgSlug, formData);
  ...
  const handleDelete = async () => {
    ...
    setIsSubmitting(true);
    try {
      await deleteEmploymentType(orgSlug, deletingItem.id);
  ...
  const totalEmployees = employmentTypes.reduce(
    (sum, type) => sum + (type._count?.employees || 0),
    0
  );
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-1">Manage employment categories for your organization</p>
          </div>
          <Button
        ...
        <div className="flex items-center justify-between gap-6 px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg">
          ...
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
              ...
              <span className="text-lg font-semibold text-gray-900">{employmentTypes.length}</span>
            </div>
            <div className="h-4 w-px bg-gray-300" />
            <div className="flex items-center gap-2">
              <span className="text-sm text-gray-600">Active:</span>
              <span className="text-lg font-semibold text-green-600">
                {employmentTypes.filter((t) => t.isActive).length}
              </span>
            </div>
            <div className="h-4 w-px bg-gray-300" />
            <div className="flex items-center gap-2">
                  ...
                  {filteredEmploymentTypes.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={6} className="text-center py-8 text-gray-500">
                        {searchQuery ? 'No employment types found matching your search.' : 'No employment types found. Create one to get started.'}
                      </TableCell>
                    </TableRow>
                  ) : (
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <form onSubmit={handleSubmit}>
            <DialogHeader>
              <DialogTitle>
                {editingItem ? 'Edit Employment Type' : 'Add New Employment Type'}
              </DialogTitle>
              <DialogDescription>
                {editingItem
                  ? 'Update employment type information'
                  : 'Add a new employment type to your organization'}
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  maxLength={20}
                />
                <p className="text-xs text-gray-500">Optional unique identifier</p>
              </div>
            ...
            <DialogDescription>
              ...
              {deletingItem?._count?.employees && deletingItem._count.employees > 0 ? (
                <span className="block mt-2 text-red-600">
                  This employment type has {deletingItem._count.employees} employee(s) assigned and cannot be deleted.
                </span>
              ) : (
