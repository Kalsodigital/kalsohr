...
export function EditUserDialog({
  ...
  useEffect(() => {
    if (open && user) {
      ...
      // Fetch roles - platform roles for superadmin/platform users, org roles for org users
      const fetchRoles = async () => {
        try {
          setLoadingRoles(true);
          let roles: Role[];
          if (user.organization) {
            // User belongs to an organization - fetch org roles
            roles = await getOrganizationRoles(user.organization.id);
          } else {
            // Platform user (no organization) - fetch platform roles
            roles = await getPlatformRoles();
          }
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 pt-4">
          ...
          {user.isSuperAdmin && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 flex items-start gap-3">
              ...
              <div>
                <h4 className="text-sm font-semibold text-yellow-800">Super Admin User</h4>
                <p className="text-xs text-yellow-700 mt-1">
                  Super admin users cannot be edited through this interface for security reasons.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="space-y-4 bg-gray-50 border border-gray-200 rounded-lg p-4">
            ...
            <p className="text-xs text-gray-500 mt-2">
              Email and organization cannot be changed after user creation
            </p>
          </div>
          ...
          <div className="space-y-4 pt-4 border-t border-gray-100">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                disabled={isSubmitting || user.isSuperAdmin || loadingRoles}
              >
                <SelectTrigger className="h-11">
                  <SelectValue placeholder={
                    loadingRoles ? "Loading roles..." : "Select role (optional)"
                  } />
                </SelectTrigger>
                <SelectContent>
              ...
              <p className="text-xs text-gray-500">
                Role determines user permissions within their organization
              </p>
            </div>
          </div>
